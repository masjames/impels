# impels — Implementation-Ready Build Spec (Beta)
...
impels is an **Android reminder app for interrupted people**. Core promise (verbatim from the
landing page):
...
The difference vs a to-do app: a normal to-do sits there quietly. impels **rings like a full-screen
alarm** at the time you set, so the task is impossible to ignore.

Three product pillars:
1. **Catch it fast** — two-tap capture: what + who asked + when to remind me.
2. **Stay focused** — one active item shown big at the top, the rest wait quietly below.
3. **It rings** — at the chosen time it fires a full-screen alarm (sound + vibration, over the lock
   screen), like the native clock/timer. Dismiss or Snooze.

---

> ## ⚠️ BEHAVIOR (authoritative — supersedes any "nag loop" wording below)
> impels is a **one-shot ringing timer**, NOT a repeat-nag loop.
> - Creating a reminder = "remind me **in** X" (5/10/15/30/60 min). It fires **once**.
> - On fire: full-screen `AlarmActivity` over the lock screen + looping alarm sound + vibration.
> - **Dismiss** → mark done, stop. **Snooze +5/+10/+15** → schedule one more one-shot. No auto-repeat.
> - Implemented in `schedule/AlarmReceiver.kt` (fires once, no reschedule), `schedule/AlarmActivity.kt`
>   (ring screen), `schedule/Notifications.kt` (`showAlarm`, full-screen intent, alarm channel).
> Sections §7/§8/§13/§16 below still describe the older repeat-nag design; where they conflict, THIS
> block and the code win.
...
```
app/src/main/java/app/impels/
  ImpelsApp.kt                 // Application, builds AppContainer, creates notif channel
  ...
  data/
    ...
    ImpelsDatabase.kt          // Room @Database
...
`data/ImpelsDatabase.kt` — `@Database(entities=[ReminderEntity::class], version=1)`, expose
`reminderDao()`. Build with `Room.databaseBuilder(...,"impels.db").build()`.
...
class ReminderScheduler(private val context: Context) {
    ...
    private fun pendingIntent(id: Long): PendingIntent {
        val i = Intent(context, AlarmReceiver::class.java).apply {
            action = "app.impels.NAG"; putExtra("id", id)
        }
...
- Channel id `"nags"`, name "Reminders", **IMPORTANCE_HIGH**, create in `ImpelsApp.onCreate()`.
...
3. Show a one-time dismissible banner suggesting to disable battery optimization
   ...
   impels to run so nags don't die." Never block the app on this.
...
Layout top→bottom:
- **Top bar**: left = logo (timer icon + "impels"), right = Settings icon button.
...
- Battery banner: "Some phones kill background apps. Allow impels to run so nags don't die."
- Settings About: "impels 0.1.0-beta · by 0xruzk".
